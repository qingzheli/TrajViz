...
public abstract class SAXSymbol {
  ...
  public boolean isNonTerminal() {
	/*  if (this.value.equalsIgnoreCase("null Guard."))
	{
		  System.out.println("isNonTerminal:    " + this.value.toString());
		  return true;
	}
	*/
	 return false;
  ...
  public void match(SAXSymbol theDigram, SAXSymbol matchingDigram) {
    ...
    if (rule.first() ==null ){
    	System.out.println("Null rule.first()");
    }
    else 
    	{
    	if (!(rule.first() ==null)&&rule.first().isNonTerminal() && (((SAXNonTerminal) rule.first()).r.count == 1))
